...
def print_function():
  bill_list = []
  with open("Planner/Bills.txt", "r") as file:
    bills = file.read().splitlines()
    for bill in bills:
      bill_items = bill.split()
      bill_class = Bill(bill_items[0], float(bill_items[1]), bill_items[2],bill_items[3])
...
def create_bill():
  ...
  while True:
    ...
    while True:
      ...
      with open("Planner/Bills.txt", "a") as file:
        file.write(f"{bill.name} {str(bill.amount)} {bill.date} {bill.paid}\n")
...
def view_bills():
  ...
  for item in bills:
    item.name = ''.join(' ' + char if char.isupper() else char.strip()
                        for char in item.name).strip()
    print(
        f"\n-- Account: {item.name}\n-- Amount Due: ${item.amount}\n-- Due Date: {item.date}\n-- Status: {item.paid}"
    )

def search_bill_name():
  found_name = []
  bills = print_function()
  while True:
    search = input(
...
def search_bill_status():
  ...
  while not complete:
    ...
    if bill_status_command == "1":
      for item in bills:
        item.name = ''.join(' ' + char if char.isupper() else char.strip() for char in item.name).strip()
        if item.paid == "Paid":
          print(
              f"\n-- Account: {item.name}\n-- Amount Due: ${item.amount}\n-- Due Date: {item.date}\n-- Status: {item.paid}"
          )
...
def pay_bill():
  ...
  while True:
    ...
    if check == True:
      ...
      command = ''.join(' ' + char if char.isupper() else char.strip()
                        for char in command).strip()
      with open("Planner/Bills.txt", "r") as file:
        bills = file.read().splitlines()
        for bill in bills:
          bill_items = bill.split()
          bill_class = Bill(bill_items[0], float(bill_items[1]), bill_items[2],
                            bill_items[3])
      ...
      for bill in bills_list:
        if bill.name == command:
          if bill.paid == "False":
            ...
            print("\nBill already paid!\n\n")
            return
        with open("Planner/Bills.txt", "w") as file:
          for bill in bills_list:
            file.write(
...
def load_event():
  if os.stat("Planner/Events.txt").st_size == 0:
    return {}
  else:
    current_events = {}
    with open("Planner/Events.txt", "r") as user:
      line = user.readlines()
      for item in line:
...
def save_event(date_dic):
  with open("Planner/Events.txt", "w") as file:
...
def show_events():
  my_file = "Planner/Events.txt"
  if os.stat(my_file).st_size == 0:
    print(Back.CYAN + "Nothing on the Calander\n" + Back.RESET)
  else:
...
def main():
  running = True
  while running:
    user_input = input(""" Would you like to open 
    ...
    #kera code
      ...
      print(Back.RED + "Invalid input. Please chooose correct option!" +Back.RESET)
